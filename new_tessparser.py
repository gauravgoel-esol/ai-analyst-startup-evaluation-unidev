...
# --- Tesseract CMD Path Configuration ---
#    - .strip() removes whitespace, .strip("'\"") removes quotes if present.
#    - os.path.normpath() normalizes path separators (e.g., / to \) for the OS.
raw_tesseract_cmd_env = os.environ.get('TESSERACT_CMD_PATH', "").strip().strip("'\"")
tesseract_cmd_path = os.path.normpath(raw_tesseract_cmd_env) if raw_tesseract_cmd_env else ""

if tesseract_cmd_path and os.path.exists(tesseract_cmd_path):
    pytesseract.pytesseract.tesseract_cmd = tesseract_cmd_path
    print(f" Using Tesseract from TESSERACT_CMD_PATH: {tesseract_cmd_path}")
else:
    # 2. If environment variable is not set or path is invalid, try a hardcoded local path
    local_tesseract_path = r'C:\Users\GAURAV D\AppData\Local\Programs\Tesseract-OCR\tesseract.exe'
    normalized_local_tesseract_path = os.path.normpath(local_tesseract_path)

    if os.path.exists(normalized_local_tesseract_path):
        pytesseract.pytesseract.tesseract_cmd = normalized_local_tesseract_path
        print(f" Using Tesseract from hardcoded local Windows path: {normalized_local_tesseract_path}")
    else:
        # 3. If neither is found, fall back to pytesseract's default behavior (checking system PATH)
        print(" Warning: Tesseract path not explicitly set via ENV, hardcoded path not found, or invalid. ")
        print("         pytesseract will attempt to find 'tesseract' automatically in system PATH, which may fail.")
        
# --- Input File Path Configuration ---
# 1. Try to get input file path from environment variable (preferred for deployment, e.g., GCS URI or mounted path)
# 2. Fallback to a hardcoded local Windows path for local development
raw_file_path = os.environ.get(
    'INPUT_FILE_PATH',
    r'C:\Users\GAURAV D\Desktop\ai_startup_hackathon\Company Data\05. Cashvisory\Cashvisory Deck_Sep 2025.pdf'
)
file_path = os.path.normpath(raw_file_path.strip().strip("'\""))
print(f" Input file path: {file_path}")

# --- Output Folder Configuration ---
# 1. Try to get output folder from environment variable
raw_output_folder = os.environ.get('OUTPUT_FOLDER', 'processed_output')
output_folder = os.path.normpath(raw_output_folder.strip().strip("'\""))
...
def extract_full_text_from_document(file_path, use_ocr_for_all_pages=True):
    ...
    try:
        if file_path.endswith('.pdf'):
            ...
            print(f" Opened PDF with {len(doc)} pages") # Added print for document opening

            for page_num, page in enumerate(doc, start=1):
                print(f" Processing page {page_num}...") # Added print for page processing
                ...
                final_page_text = clean_ocr_text("\n".join(final_page_text_parts)) # Apply final clean to combined text
...
if __name__ == "__main__":
    if not os.path.exists(file_path):
        print(f" ERROR: Input file not found at '{file_path}'. Please ensure the file is accessible.")
        exit(1) # Exit if the input file isn't found
    ...
    if pages_dict:
        ...
        print(" No text extracted or processed from document.")